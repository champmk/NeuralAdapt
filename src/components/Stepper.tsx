...
type StepperProps = HTMLAttributes<HTMLDivElement> & {
  ...
  onStepNext?: (step: number) => Promise<boolean> | boolean;
};
...
export default function Stepper({
  ...
  const goNext = useCallback(async () => {
    const shouldAdvance = (await onStepNext?.(currentStep)) ?? true;
    if (!shouldAdvance) {
      return false;
    }
    setDirection(1);
    updateStep(currentStep + 1);
    return true;
  }, [currentStep, onStepNext, updateStep]);

  const isCompleted = currentStep > totalSteps;
  const isLastStep = currentStep === totalSteps;

  const contextValue = useMemo(
    () => ({ currentStep, totalSteps, goToStep, goNext, goBack, isCompleted }),
    [currentStep, totalSteps, goToStep, goNext, goBack, isCompleted],
  );
